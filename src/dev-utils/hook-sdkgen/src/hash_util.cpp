...
// Public domain SHA256 from https://github.com/B-Con/crypto-algorithms (Brad Conte)
namespace {
struct SHA256_CTX {
    uint8_t data[64];
    uint32_t datalen;
...
static void sha256_update(SHA256_CTX *ctx, const uint8_t data[], size_t len) {
    for (size_t i=0;i<len;++i){ ctx->data[ctx->datalen]=data[i]; ctx->datalen++; if(ctx->datalen==64){ sha256_transform(ctx,ctx->data); ctx->bitlen+=512; ctx->datalen=0; } }
}
static void sha256_final(SHA256_CTX *ctx, uint8_t hash[]) {
    ...
    ctx->bitlen+=ctx->datalen*8; ctx->data[63]=ctx->bitlen; ctx->data[62]=ctx->bitlen>>8; ctx->data[61]=ctx->bitlen>>16; ctx->data[60]=ctx->bitlen>>24; ctx->data[59]=ctx->bitlen>>32; ctx->data[58]=ctx->bitlen>>40; ctx->data[57]=ctx->bitlen>>48; ctx->data[56]=ctx->bitlen>>56; sha256_transform(ctx,ctx->data);
...
std::array<uint8_t,32> sha256(const uint8_t* data, size_t len) {
    SHA256_CTX ctx; sha256_init(&ctx); sha256_update(&ctx, data, len); std::array<uint8_t,32> out{}; sha256_final(&ctx, out.data()); return out;
}
std::array<uint8_t,32> sha256(const std::vector<uint8_t>& data){ return sha256(data.data(), data.size()); }
std::array<uint8_t,32> sha256(const std::string& s){ return sha256(reinterpret_cast<const uint8_t*>(s.data()), s.size()); }
std::string sha256_hex(const std::array<uint8_t,32>& h){
    std::ostringstream oss; oss<<std::hex<<std::setfill('0'); for(auto b: h) oss<<std::setw(2)<<(int)b; std::string r=oss.str(); for(char&c:r) c=tolower((unsigned char)c); return r;
}
